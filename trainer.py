...
class BrainNetworkTrainer:
    ...
    def train_epoch(self, dataloader, epoch_idx, progress_bar=True):
        ...
        for batch_idx, (features, labels) in enumerate(pbar):
            ...
            # 计算损失
            total_loss, loss_components = self.criterion(predictions, labels, model_outputs)
...
def train_complete_model(config):
    ...
    for epoch in range(config['num_epochs']):
        ...
        if current_auc > best_val_auc:
            ...
            trainer.save_checkpoint(epoch, config.get('best_model_path', 'best_model.pth'))
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'learning_rate': 1e-4,
        'train_ratio': 0.8,
        'best_model_path': 'best_brain_network_model.pth',
        'save_interval': 20
    }
